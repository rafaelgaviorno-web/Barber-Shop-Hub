...
export default function BarbershopDetail() {
  ...
  if (!shop) {
    return (
      <ClientLayout>
        <div className="text-center py-12">Barbershop not found.</div>
      </ClientLayout>
    );
  }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border pb-8">
          <div>
            ...
            <div className="flex flex-wrap gap-4 mt-4 text-muted-foreground">
              {shop.address && (
                <span className="flex items-center gap-1.5"><MapPin className="h-4 w-4" /> {shop.address}</span>
              )}
              {shop.phone && (
                <span className="flex items-center gap-1.5"><Phone className="h-4 w-4" /> {shop.phone}</span>
              )}
              {(shop.openTime || shop.closeTime) && (
                <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> {shop.openTime} - {shop.closeTime}</span>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <h2 className="text-2xl font-semibold flex items-center gap-2">
              <Scissors className="h-6 w-6 text-primary" /> Services
            </h2>
            {servicesLoading ? (
              <div>Loading services...</div>
            ) : services?.length === 0 ? (
              <div className="text-muted-foreground p-4 bg-card rounded-md border border-border">No services listed.</div>
            ) : (
          ...
          <div className="space-y-4">
            <h2 className="text-2xl font-semibold flex items-center gap-2">
              <User className="h-6 w-6 text-primary" /> Barbers
            </h2>
            {barbersLoading ? (
              <div>Loading barbers...</div>
            ) : barbers?.length === 0 ? (
              <div className="text-muted-foreground p-4 bg-card rounded-md border border-border">No barbers listed.</div>
            ) : (
                ...
                {barbers?.map(barber => (
                  <Card key={barber.id} className="bg-card">
                    <CardHeader className="py-4">
                      ...
                      {barber.specialties && (
                        <CardDescription className="mt-1">{barber.specialties}</CardDescription>
                      )}
                    </CardHeader>
                  </Card>
                ))}
